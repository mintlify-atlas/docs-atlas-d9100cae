...
description: "How to register a strategy with addStrategySchema, implement getSignal with ladder-averaging logic, and react to active pings and errors via event listeners."
---
...
<ParamField path="getSignal" type="function" required>
  ...
  <Expandable title="getSignal return shape (SignalResult)">
    ...
    <ResponseField name="minuteEstimatedTime" type="number" required>
      Estimated hold time in minutes. Use `Infinity` in the test file to let the ladder logic drive the close, or a fixed number to impose a time-based exit.
    </ResponseField>
