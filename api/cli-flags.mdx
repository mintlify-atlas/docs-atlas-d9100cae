---
title: "CLI Flags Reference for @pro/main Entry Points"
sidebarTitle: "CLI Flags"
description: "Every boolean flag parsed by getArgs(), the positional strategy-file argument, and the environment variables that control parallel-mode symbol lists and Telegram auth."
---
...
<ParamField path="--ui" type="boolean" default="false">
  Starts the `@backtest-kit/ui` web interface on port **60050**. Passed through to `@backtest-kit/cli`; the dashboard shows live equity curves, open positions, and log streams for all running symbols.
</ParamField>

<ParamField path="--noCache" type="boolean" default="false">
  Explicitly disables candle caching even when the environment would otherwise attempt it. Passed through to `@backtest-kit/cli`. Useful in Mode B when you want to force a fresh fetch from the exchange on every run.
</ParamField>
